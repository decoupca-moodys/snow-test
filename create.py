...
if not incident_exists(short_description):
    new_record = {"short_description": short_description, "description": description}
    result = incident.create(payload=new_record)
    if result:
        print("Created new incident!")
else:
    print(
        f'Incident with short description "{short_description}" already exists, doing nothing'
    )
